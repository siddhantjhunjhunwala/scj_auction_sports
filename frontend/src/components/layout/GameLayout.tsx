...
export default function GameLayout({ children }: GameLayoutProps) {
  ...
  const isActivePath = (path: string) => {
    return location.pathname.includes(`/game/${gameId}/${path}`);
  };
      ...
      <aside className="w-64 bg-[var(--bg-secondary)] border-r border-[var(--glass-border)] flex flex-col fixed h-full z-40">
        ...
        <div className="p-4 border-t border-[var(--glass-border)]">
          ...
          <button
            ...
            className="w-full text-left px-3 py-2 text-sm text-[var(--text-tertiary)] hover:text-[var(--accent-red)] hover:bg-[var(--bg-tertiary)] rounded-lg transition-colors"
          >
            Logout
          </button>
        </div>
